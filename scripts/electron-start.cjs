...
const oauth = loadLocalYouTubeOAuthConfig(); const supabase = loadLocalSupabaseConfig(); const env = { ...process.env, ...(oauth.clientId ? { SHORTSFLOW_YOUTUBE_CLIENT_ID: oauth.clientId } : {}), ...(oauth.clientSecret ? { SHORTSFLOW_YOUTUBE_CLIENT_SECRET: oauth.clientSecret } : {}), ...(supabase.url ? { VITE_SUPABASE_URL: supabase.url } : {}), ...(supabase.anonKey ? { VITE_SUPABASE_ANON_KEY: supabase.anonKey } : {}) };
