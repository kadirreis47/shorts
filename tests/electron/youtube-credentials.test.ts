...
const { createCredentialVault: createOwnedCredentialVault, createPkce, createState } = require('../../electron/youtube-credentials.cjs') as { createCredentialVault: (input: any) => any; createPkce: () => { verifier: string; challenge: string }; createState: () => string };
...
const testOwnerContext = {
  capture: () => ({ ownerId: TEST_OWNER_ID, generation: 1, signal: ownerSignal }),
  assertCurrent: (context: { ownerId: string; generation: number }) => { if (context?.ownerId !== TEST_OWNER_ID || context?.generation !== 1) throw new Error('stale owner'); },
  assertCriticalCurrent: (context: { ownerId: string; generation: number }) => { if (context?.ownerId !== TEST_OWNER_ID || context?.generation !== 1) throw new Error('stale owner'); },
  runCritical: async (_context: unknown, operation: () => unknown) => operation(),
};
const createCredentialVault = (input: any) => createOwnedCredentialVault({ ...input, ownerContext: input.ownerContext ?? testOwnerContext });
...
describe('YouTube credential boundary', () => {
  ...
  it('rejects credential storage failures after a successful callback without exposing OAuth secrets', async () => {
    const vault = { ...testOwnerContext, captureOwnerContext: testOwnerContext.capture, assertOwnerContext: testOwnerContext.assertCurrent, store: vi.fn(async () => { throw Object.assign(new Error('storage failed'), { code: 'credential-storage-failed' }); }) };
  ...
  it('removes only terminally invalid credentials, fails status closed, and never retries the dead refresh token', async () => { directory = await mkdtemp(join(tmpdir(), 'shortsflow-youtube-')); const vault = createCredentialVault({ userDataPath: directory, safeStorage }); const ref = await vault.store({ ...credential, expiresAt: new Date(Date.now() - 1).toISOString() }); const unaffectedRef = await vault.store({ ...credential, channelId: 'UC-other' }); const fetchImpl = vi.fn(async () => ({ ok: false, status: 400, json: async () => ({ error: 'invalid_grant' }) })); const service = createYouTubeAuthService({ vault, clientId: 'test-client', clientSecret: SYNTHETIC_CLIENT_SECRET, fetchImpl }); await expect(service.resolveExecutionCredential(ref)).rejects.toMatchObject({ code: 'credential-reconnect-required' }); await expect(service.status(ref)).rejects.toMatchObject({ code: 'credential-unavailable' }); await expect(service.resolveExecutionCredential(ref)).rejects.toMatchObject({ code: 'credential-unavailable' }); expect(fetchImpl).toHaveBeenCalledTimes(1); expect(await vault.resolve(unaffectedRef)).toEqual(expect.objectContaining({ channelId: 'UC-other' })); });
  ...
  it('generates an explicitly unconfigured runtime file when OAuth is absent and clears stale credential values', async () => { directory = await mkdtemp(join(tmpdir(), 'shortsflow-youtube-')); const output = join(directory, 'shortsflow.runtime.json'); const noSupabase = () => ({ url: null, anonKey: null }); generateRuntimeConfig({ clientId: 'synthetic-release-client', clientSecret: SYNTHETIC_CLIENT_SECRET, resolveSupabaseConfig: noSupabase, outputFile: output }); expect(JSON.parse(await readFile(output, 'utf8'))).toEqual({ youtubeClientId: 'synthetic-release-client', youtubeClientSecret: SYNTHETIC_CLIENT_SECRET, supabaseUrl: '', supabaseAnonKey: '' }); generateRuntimeConfig({ outputFile: output, resolveConfig: () => ({ clientId: null, clientSecret: null }), resolveSupabaseConfig: noSupabase }); expect(JSON.parse(await readFile(output, 'utf8'))).toEqual({ youtubeClientId: '', youtubeClientSecret: '', supabaseUrl: '', supabaseAnonKey: '' }); expect(resolveYouTubeClientId({ env: {}, userDataPath: directory, resourcesPath: join(directory, 'none') })).toBeNull(); });
  it('uses one atomic local credential resolver for build generation with environment precedence', async () => { directory = await mkdtemp(join(tmpdir(), 'shortsflow-youtube-')); const output = join(directory, 'shortsflow.runtime.json'); const fsApi = { readFileSync: () => `SHORTSFLOW_YOUTUBE_CLIENT_ID=from-dotenv\nSHORTSFLOW_YOUTUBE_CLIENT_SECRET=${SYNTHETIC_CLIENT_SECRET}\n` }; const fromDotEnv = loadLocalYouTubeOAuthConfig({ env: {}, fsApi }); const fromEnvironment = loadLocalYouTubeOAuthConfig({ env: { SHORTSFLOW_YOUTUBE_CLIENT_ID: 'from-environment', SHORTSFLOW_YOUTUBE_CLIENT_SECRET: 'environment-test-secret' }, fsApi }); expect(fromDotEnv).toEqual({ clientId: 'from-dotenv', clientSecret: SYNTHETIC_CLIENT_SECRET }); expect(fromEnvironment).toEqual({ clientId: 'from-environment', clientSecret: 'environment-test-secret' }); generateRuntimeConfig({ outputFile: output, resolveConfig: () => fromEnvironment, resolveSupabaseConfig: () => ({ url: 'https://project.supabase.co', anonKey: 'public-anon-key' }) }); expect(JSON.parse(await readFile(output, 'utf8'))).toEqual({ youtubeClientId: 'from-environment', youtubeClientSecret: 'environment-test-secret', supabaseUrl: 'https://project.supabase.co', supabaseAnonKey: 'public-anon-key' }); });
