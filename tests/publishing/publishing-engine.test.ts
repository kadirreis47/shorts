...
const artifact = { path: 'C:/exports/video.mp4', sizeBytes: 100, durationMs: 1000, contentDigest: 'a'.repeat(64), verifiedExportReference: `vea1_${'A'.repeat(43)}`, verified: true, diagnostics: {}, createdAt: 'now' };
...
describe('Epic 7.8 publishing engine', () => {
  ...
  it('routes the real YouTube adapter through the narrow trusted publishing client', async () => { const publish = vi.fn(async (_request: any) => ({ ok: true as const, result: { remotePublishId: 'video-1', remoteUrl: 'https://www.youtube.com/watch?v=video-1', state: 'published' as const } })); const client = { publish, reconcilePublish: vi.fn(), cancelPublish: vi.fn(), acknowledgeReceipt: vi.fn() } as any; const adapter = createPublishAdapterRegistry(client).get('youtube'); await expect(adapter.publish({ job: baseJob(), signal: new AbortController().signal, report: vi.fn() })).resolves.toMatchObject({ remotePublishId: 'video-1' }); const payload = publish.mock.calls[0][0]; expect(payload).toMatchObject({ jobId: 'job-1', platform: 'youtube', account: { credentialRef: 'credential-ref', channelRef: 'c1' }, artifact: { verifiedExportReference: artifact.verifiedExportReference, artifactFingerprint: 'artifact-1' } }); expect(payload.artifact).not.toHaveProperty('artifactPath'); expect(payload.artifact).not.toHaveProperty('contentDigest'); expect(payload.artifact).not.toHaveProperty('sizeBytes'); expect(JSON.stringify(payload)).not.toContain('accessToken'); expect(JSON.stringify(payload)).not.toContain('sessionUri'); });
  ...
  it('keeps ephemeral publish capability rotation outside durable approval identity', () => { const job = baseJob(); expect(approvalFingerprint({ ...job, artifact: { ...job.artifact, verifiedExportReference: `vea1_${'B'.repeat(43)}` } })).toBe(approvalFingerprint(job)); });
