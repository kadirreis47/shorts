...
export function validateAnalysisImage(path: string, contentType: string, bytes: Uint8Array): AnalysisImageContentType {
  ...
  if (path.endsWith(".png") && contentType === "image/png" && validPng(bytes)) return "image/png";
  if (path.endsWith(".jpg") && contentType === "image/jpeg" && validJpeg(bytes)) return "image/jpeg";
  throw new Error("invalid-image");
}

function validPng(bytes: Uint8Array): boolean {
  if (bytes.length < 57 || !PNG_SIGNATURE.every((byte, index) => bytes[index] === byte) || uint32(bytes, 8) !== 13 || ascii(bytes, 12, 16) !== "IHDR") return false;
  const width = uint32(bytes, 16); const height = uint32(bytes, 20);
  if (!dimensions(width, height) || !validPngColor(bytes[24], bytes[25]) || bytes[26] !== 0 || bytes[27] !== 0 || (bytes[28] !== 0 && bytes[28] !== 1)) return false;
  if (uint32(bytes, 29) !== crc32(bytes.slice(12, 29))) return false;
  ...
  while (offset < bytes.length && chunks < MAX_PNG_CHUNKS) {
    ...
    if (uint32(bytes, offset + 8 + length) !== crc32(bytes.slice(offset + 4, offset + 8 + length))) return false;
    if (type === "IHDR") return false;
    if (type === "IDAT") { if (length < 1) return false; sawIdat = true; }
...
function validJpeg(bytes: Uint8Array): boolean {
  if (bytes.length < 6 || bytes[0] !== 0xff || bytes[1] !== 0xd8 || bytes[bytes.length - 2] !== 0xff || bytes[bytes.length - 1] !== 0xd9) return false;
  let offset = 2; let segments = 0; let headerBytes = 0; let sawSof = false; let sawScan = false;
  while (offset < bytes.length && segments < MAX_JPEG_SEGMENTS) {
    if (bytes[offset] !== 0xff) return false;
    while (offset < bytes.length && bytes[offset] === 0xff) offset += 1;
    if (offset >= bytes.length) return false;
    const marker = bytes[offset++]; segments += 1;
    if (marker === 0xd9) return sawSof && sawScan && offset === bytes.length;
    ...
    if (headerBytes > MAX_JPEG_HEADER_BYTES) return false;
    if (isSof(marker)) {
      const components = bytes[offset + 7];
      if (length < 11 || components < 1 || length !== 8 + components * 3) return false;
    ...
    if (marker === 0xda) {
      ...
      if (!sawSof || length < 8 || components < 1 || length !== 6 + components * 2) return false;
