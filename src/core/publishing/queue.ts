...
import { validateExecutablePublishBinding } from './binding';
...
export function canRetryPublishJob(job: PublishJob, now = new Date()): boolean { return job.state === 'failed' && getPublishJobExecutionEligibility(job, now).runnable; }
...
export function createPublishQueue(executor: PublishExecutor, update?: (job: PublishJob) => void): PublishQueue {
  let snapshot: PublishQueueSnapshot = { jobs: [], activeJobId: null, paused: false }; let running = false; let nextWakeAt: number | null = null; let wakeTimer: ReturnType<typeof setTimeout> | null = null; const cancelled = new Set<string>();
  ...
  const process = async (job: PublishJob) => {
    ...
    const attempt: PublishAttempt = { id: stableId('publish-attempt', `${job.id}:${job.attempts.length + 1}:${Date.now()}`), attempt: job.attempts.length + 1, startedAt: new Date().toISOString(), endedAt: null, idempotencyKey: job.idempotencyKey, remotePublishId: job.remotePublishId };
    const current: PublishJob = { ...job, state: isReconciling ? 'reconciling' : 'uploading', attempts: [...job.attempts, attempt], updatedAt: new Date().toISOString() }; save(current);
    try {
      if (isReconciling) {
        ...
        if (result.state === 'reconciling' || result.state === 'interrupted' || result === before) {
          const attempts = (result.reconciliationAttempts ?? 0) + 1; if (attempts >= 3) { save({ ...result, state: 'failed', reconciliationAttempts: attempts, failure: { kind: 'unknown', code: 'reconciliation-unresolved', message: 'Remote publication state could not be resolved.', retryable: false, attempt: result.attempts.length, maxAttempts: result.maxAttempts, retryAfterUtc: null, stderrTail: [] }, updatedAt: new Date().toISOString() }); return; }
          save({ ...result, state: 'reconciling', reconciliationAttempts: attempts, nextReconcileAt: calculateRetryAt(attempts, 30_000, 300_000, 1_000), progress: { ...result.progress, state: 'reconciling', message: 'Remote state unresolved; reconciliation is backed off.', updatedAt: new Date().toISOString() }, updatedAt: new Date().toISOString() }); return;
        }
        save(result); return;
      }
      if (!current.approvalFingerprint) throw Object.assign(new Error('Publish preview required before execution.'), { retryable: false, status: 400 });
      ...
      const receipt = await executor.run(current, (progress) => save({ ...current, progress })); if (!receipt.verification.valid || receipt.verification.remoteState !== 'published') throw new Error('Remote publication was not verified as published.');
