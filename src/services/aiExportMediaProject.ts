...
import { hydrateTrustedImageGeometry } from '@/lib/imageGeometryHydration';
...
export async function buildAIExportStudioMediaProject(
  ...
  const hydration = await hydrateTrustedImageGeometry(canonicalScenes, resolveGeometry, { force: true });
  if (hydration.failedMedia.length > 0) throw new Error('Trusted image geometry is unresolved. Retry after private media becomes available.');
  const resolvedScenes: Scene[] = hydration.scenes;
