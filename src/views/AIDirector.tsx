...
export function AIDirector({ onNavigateEditor, onNavigateStudio }: { onNavigateEditor?: () => void; onNavigateStudio: () => void }) {
  ...
  if (!report) return <Card className="mx-auto max-w-xl p-8 text-center"><BrainCircuit className="mx-auto text-slate-400" size={36} /><h1 className="mt-4 text-xl font-semibold">AI Director</h1><p className="mt-2 text-sm text-slate-500">Aktif Media/Render manifestini deterministic heuristic kurallarla analiz eder. ML/LLM tahmini değildir.</p>{error && <p className="mt-3 text-sm text-rose-600">{error}</p>}<Button className="mt-5" onClick={onNavigateStudio}><Play size={16} /> Analizi Başlat</Button></Card>;
  return <div className="space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-bold">AI Director Report</h1><p className="text-sm text-slate-500">Heuristic, deterministic rapor v{report.reportVersion}; edit plan otomatik uygulanmaz.</p></div><div className="flex gap-2"><Button variant="secondary" onClick={onNavigateStudio}><Play size={16} /> Studio'da Yeniden Analiz</Button>{onNavigateEditor && <Button variant="secondary" onClick={onNavigateEditor}>AI Editor Planı Oluştur</Button>}<Button onClick={() => exportReport(report)}><Download size={16} /> JSON</Button></div></div>
