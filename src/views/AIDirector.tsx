...
export function AIDirector({ onNavigateEditor }: { onNavigateEditor?: () => void }) {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-bold">AI Director Report</h1><p className="text-sm text-slate-500">Heuristic, deterministic rapor v{report.reportVersion}; edit plan otomatik uygulanmaz.</p></div><div className="flex gap-2"><Button variant="secondary" onClick={() => void run()}><Play size={16} /> Yeniden Analiz</Button>{onNavigateEditor && <Button variant="secondary" onClick={onNavigateEditor}>AI Editor Planı Oluştur</Button>}<Button onClick={() => exportReport(report)}><Download size={16} /> JSON</Button></div></div>
